...
export default function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled) { setAgentCampaigns([]); const inferredContext = campaignSubject?.userCategory === 'brand_ambassador' ? 'merchant-educational' : 'vodacom-privilege'; setActiveCampaign(inferredContext); localStorage.setItem('btl_active_campaign', inferredContext); setActiveCampaignPause(null); }
      }
    })();
  ...
  const refreshData = useCallback(async (force = false) => {
    ...
    if (cacheIsFresh) { setUsers(getUsers()); setShops(getShops()); return; }
  ...
  const isMerchantContext = !isMikiliContext && !isYouthContext && (effectiveRole === 'agent' ? (agentCampaignOptions.length > 0 ? activeCampaign === 'merchant-educational' : inferredAgentMerchant) : activeCampaign === 'merchant-educational');
  ...
  return (<div className="app-shell h-screen flex flex-col relative overflow-hidden font-sans select-none transition-colors" style={{ color: themeSurfaceStyle.color }}>
    ...
    <BottomNav userRole={effectiveRole} activeTab={activeTab} unreadChatCount={chatUnreadCount} merchantContext={isMerchantContext} youthContext={isYouthContext} mikiliContext={isMikiliContext} onTabChange={(tab) => { if (tab === 'home') setHomeTabPressCount((prev) => prev + 1); setActiveTab(tab); }} />
