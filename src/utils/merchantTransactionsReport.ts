...
export const exportMerchantTransactionsPdf = async (input: MerchantTransactionsReportInput): Promise<void> => {
  ...
  const headers = ['Date', 'BA', 'Short-code POS', 'POS', 'MFS', 'Client', 'Référence', 'Montant'];
  const widths = [24, 31, 25, 43, 30, 30, 40, 30];
  const rowHeight = 8;
  ...
  input.records.forEach((record, index) => {
    ...
    const cells = [compactDate(record.occurred_at), record.ba?.name || '—', record.point_of_sale?.agent_number || '—', record.point_of_sale?.denomination || '—', record.point_of_sale?.mfs_name || '—', record.client_number || '—', record.transaction_reference || '—', money(record.amount)];
