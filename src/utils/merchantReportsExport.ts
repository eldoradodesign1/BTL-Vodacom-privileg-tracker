...
import { cleanReportComment } from './reportComment';
...
export async function exportMerchantAgentReportPdf(input: MerchantAgentReportInput): Promise<void> {
  ...
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(9); pdf.setTextColor(8, 51, 68); pdf.text('COMMENTAIRE DE CLÔTURE', margin, y); y += 6; lines(cleanReportComment(input.attendance.closing_comment) || 'Aucun commentaire renseigné.');
...
export async function exportMerchantAgentReportExcel(input: MerchantAgentReportInput): Promise<void> {
  ...
  transactions.forEach((item) => tx.addRow([item.occurred_at, item.point_of_sale?.denomination || item.pos_id, item.point_of_sale?.agent_number || '', item.client_number || '', item.transaction_reference || '', Number(item.amount || 0), input.attendance.mfs_name || '', cleanReportComment(item.comment)]));
