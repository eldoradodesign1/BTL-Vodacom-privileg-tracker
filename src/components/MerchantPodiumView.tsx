...
export const MerchantPodiumView: React.FC = () => {
  ...
  return <div className="space-y-4 pb-4">
    <section className="glass-card relative overflow-hidden p-4 sm:p-5">
      ...
      <div className="relative flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-amber-100/80">Podium du jour</p><h1 className="mt-1 text-xl font-black">Rythme d’arrivée Merchant</h1><p className="mt-1 max-w-sm text-xs text-gray-400">À chiffres identiques, le classement suit l’heure du premier POS pointé par chaque BA.</p></div><div className="flex gap-2"><button type="button" onClick={() => setIsReportsOpen(true)} title="Rapports superviseur" className="rounded-2xl border border-cyan-300/30 bg-cyan-400/10 p-3 text-cyan-100 transition hover:bg-cyan-400/20"><FileText size={17}/></button><button type="button" onClick={() => void load()} title="Actualiser le podium" className="rounded-2xl border border-white/10 bg-white/5 p-3 text-gray-200 transition hover:bg-white/10"><RefreshCw size={17}/></button></div></div>
