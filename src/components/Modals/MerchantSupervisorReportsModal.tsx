...
export const MerchantSupervisorReportsModal: React.FC<MerchantSupervisorReportsModalProps> = ({ isOpen, run, onClose }) => {
  ...
  const [selectedDate, setSelectedDate] = useState(merchantTodayIso());
  ...
  const load = async () => {
    ...
    setError('');
    try {
      setReport(await getMerchantSupervisorReport(run, kind, selectedDate));
    } catch (caught) {
  ...
  useEffect(() => {
    if (isOpen) void load();
  }, [isOpen, kind, run?.id, selectedDate]);
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      if (comment.trim()) {
        sectionTitle('Commentaire du rapport');
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(8.5);
        pdf.setTextColor(51, 65, 85);
        const lines = pdf.splitTextToSize(comment.trim(), contentWidth - 8);
        ensureSpace((lines.length * 4.4) + 8);
  ...
  return <div className="fixed inset-0 z-[95] flex items-end justify-center bg-black/75 p-0 backdrop-blur-md sm:items-center sm:p-4" onClick={onClose}>
    <div className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-t-[2rem] border border-white/10 bg-[#0b101a] p-4 shadow-2xl sm:rounded-[2rem] sm:p-5" onClick={(event) => event.stopPropagation()}>
      ...
      {kind === 'daily' && <div className="mt-3 rounded-2xl border border-cyan-300/20 bg-cyan-400/[0.06] p-2"><DateIconPicker value={selectedDate} min={MERCHANT_CAMPAIGN_START} max={merchantTodayIso()} onChange={setSelectedDate} className="flex items-center gap-2" buttonClassName="h-10 w-10 rounded-xl border border-cyan-300/20 bg-cyan-400/[0.08] text-cyan-100 transition hover:bg-cyan-400/15" labelClassName="text-[10px] font-black uppercase text-gray-200" popoverAlign="left"/><p className="mt-1 px-1 text-[10px] font-semibold text-gray-400">Choisissez le jour précis à rapporter.</p></div>}
      <label className="mt-3 block text-[10px] font-black uppercase tracking-[0.14em] text-gray-400">Commentaire du rapport <span className="normal-case font-semibold">(optionnel)</span><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Ajoutez une synthèse ou une consigne de suivi…" className="app-input mt-2 min-h-20 w-full rounded-2xl p-3 text-sm"/></label>
      ...
      {report && !loading && <div className="mt-5 overflow-hidden rounded-[1.5rem] border border-cyan-300/15 bg-[radial-gradient(circle_at_92%_0%,rgba(34,211,238,0.16),transparent_33%),linear-gradient(145deg,#101827,#0b101a)] p-4 text-white sm:p-6">
        ...
        {comment.trim() && <section className="mt-5 rounded-2xl border border-cyan-300/15 bg-cyan-400/[0.05] p-3"><p className="text-[9px] font-black uppercase tracking-[0.14em] text-cyan-100">Commentaire du rapport</p><p className="mt-2 whitespace-pre-wrap text-xs leading-relaxed text-gray-200">{comment.trim()}</p></section>}
