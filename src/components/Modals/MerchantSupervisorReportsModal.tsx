...
export const MerchantSupervisorReportsModal: React.FC<MerchantSupervisorReportsModalProps> = ({ isOpen, run, onClose }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (existing) { setComment(existing.comment); setCommentIsAi(existing.aiGenerated); return; }
      ...
      const suggestion = await suggestSupervisorReportComment({
        ...
        metrics: commentMetrics(next), agentComments: next.agentReports.map((item) => item.comment).filter(Boolean),
      });
  ...
  const persistComment = async (): Promise<boolean> => {
    ...
    try {
      await saveSupervisorReportComment({
        ...
        aiGenerated: commentIsAi, sourceComments: report.agentReports.map((item) => item.comment).filter(Boolean), metrics: commentMetrics(report),
      });
      return true;
