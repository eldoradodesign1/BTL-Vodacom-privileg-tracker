...
export const MerchantMfsPicker: React.FC<MerchantMfsPickerProps> = ({ value, onChange, disabled = false, accent = 'cyan', availableNames = [] }) => {
  ...
  const detailDocument = { title: 'Répertoire MFS', subtitle: query.trim() ? `Résultats pour « ${query.trim()} »` : `${filtered.length} MFS affiché${filtered.length > 1 ? 's' : ''}`, filename: `mfs-${query || 'liste'}`, sections: [{ title: 'MFS affichés', rows: filtered.map((name, index) => ({ label: `#${index + 1}`, value: name })) }] };
  return <>
    ...
    {isOpen && <div className="fixed inset-0 z-[145] flex items-end bg-black/75 p-0 backdrop-blur-md sm:items-center sm:justify-center sm:p-4" onClick={() => setIsOpen(false)}>
      <section className="modal-sheet w-full max-w-xl overflow-hidden p-0 sm:rounded-3xl" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="Sélectionner un MFS">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3"><div className={`flex items-center gap-2 ${softText}`}><UserRoundSearch size={18}/><div><h2 className="text-sm font-black">Sélectionner un MFS</h2><p className="text-[10px] text-gray-400">Le choix limite la liste des POS disponibles.</p></div></div><div className="flex items-center gap-2"><DetailPdfExportButton document={detailDocument}/><button type="button" onClick={() => setIsOpen(false)} className="rounded-xl p-2 text-gray-400 transition hover:bg-white/10 hover:text-white" aria-label="Fermer"><X size={18}/></button></div></div>
