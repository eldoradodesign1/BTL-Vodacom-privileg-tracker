...
export const MerchantCompiledReportModal: React.FC<Props> = ({ isOpen, archives, startDate, endDate, onClose }) => {
  ...
  useEffect(() => {
    ...
    const prepareComment = async () => {
      ...
      try {
        ...
        if (stored?.comment && !needsFrenchRewrite(stored.comment)) { if (!cancelled) { setComment(stored.comment); setFromAi(stored.aiGenerated); } return; }
  ...
  const prepareExport = async (mode: Exclude<ExportMode, null>, includeAmounts = true) => {
    ...
    try {
      ...
      if (mode === 'xlsx') await exportMerchantCompiledExcel(completed); else await exportMerchantCompiledPdf(completed, includeAmounts);
  ...
  return <div className="fixed inset-0 z-[90] flex items-end bg-slate-950/80 p-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-5">
    <div className="flex max-h-[94dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-[2rem] border border-white/10 bg-[#071018]/95 shadow-2xl sm:rounded-[2rem]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">
        ...
        <section className="mb-4 rounded-2xl border border-cyan-300/15 bg-cyan-400/[0.05] p-4"><div className="mb-2 text-xs font-black uppercase tracking-[0.12em] text-cyan-100">{loadingComment ? 'Préparation du commentaire…' : 'Commentaire superviseur'}</div><textarea value={comment} onChange={(event) => { setComment(event.target.value); setFromAi(false); }} disabled={loadingComment} placeholder="Synthèse du superviseur…" className="app-input min-h-28 w-full resize-y rounded-xl p-3 text-sm leading-relaxed"/></section>
