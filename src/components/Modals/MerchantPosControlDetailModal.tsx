...
import { getMerchantEvidencePublicUrl, markMerchantPosVisitInactive, updateMerchantPosMfs } from '../../utils/merchantCampaign';
...
export const MerchantPosControlDetailModal: React.FC<MerchantPosControlDetailModalProps> = ({ item, transactionsPerPosTarget, onClose, onUpdated }) => {
  const [arrivalPhoto, setArrivalPhoto] = useState('');
  const [proofs, setProofs] = useState<Record<string, string>>({});
  const [lightbox, setLightbox] = useState<LightboxImage | null>(null);
  const [mfsName, setMfsName] = useState('');
  const [savingMfs, setSavingMfs] = useState(false);
  ...
  useEffect(() => {
    ...
    setProofs({});
    setLightbox(null);
    setMfsName(item.pos.mfs_name || '');
    setMfsError('');
    setInactiveNote(item.visit?.operational_note || '');
    setInactiveError('');
    void Promise.all([
  ...
  if (!item) return null;
  const saveMfs = async () => {
    setSavingMfs(true); setMfsError('');
    try { await updateMerchantPosMfs(item.pos.id, mfsName); onUpdated?.(); }
    catch (caught) { setMfsError(caught instanceof Error ? caught.message : 'Mise à jour du MFS impossible.'); }
    finally { setSavingMfs(false); }
  };
  const markInactive = async () => {
    if (!item.visit) {
      setInactiveError('Ce POS ne possède aucune arrivée à corriger.');
      return;
    }
    ...
    setSavingInactive(true); setInactiveError('');
    try {
      await markMerchantPosVisitInactive(item.visit.id, inactiveNote);
      onUpdated?.();
    } catch (caught) {
      setInactiveError(caught instanceof Error ? caught.message : 'Correction non active impossible.');
  ...
  const detailDocument = {
    title: 'Détail opérationnel POS',
    subtitle: `${item.pos.denomination} · ${statusLabel}`,
    filename: `pos-${item.pos.agent_number || item.pos.denomination}`,
    sections: [
      { title: 'Identification', rows: [{ label: 'POS', value: item.pos.denomination }, { label: 'Short-code', value: item.pos.agent_number }, { label: 'Adresse', value: item.pos.address }, { label: 'Pool', value: item.pos.pool }, { label: 'MFS', value: item.pos.mfs_name }, { label: 'Statut', value: statusLabel }] },
      { title: 'Activité terrain', rows: [{ label: 'BA', value: item.ba?.name }, { label: 'Arrivée', value: arrival }, { label: 'Transactions', value: item.status === 'inactive' ? 'Aucune transaction requise' : `${item.transactionCount}/${transactionsPerPosTarget}` }, { label: 'Précision GPS', value: item.visit?.accuracy_m != null ? `${Math.round(item.visit.accuracy_m)} m` : null }] },
      ...(item.visit?.operational_note?.trim() ? [{ title: 'Constat terrain', text: item.visit.operational_note.trim() }] : []),
      ...(item.transactions.length ? [{ title: 'Transactions réalisées', rows: item.transactions.map((transaction, index) => ({ label: `Transaction ${index + 1}`, value: `${transaction.client_number || transaction.transaction_reference || 'Sans référence'} · ${Number(transaction.amount || 0).toLocaleString('fr-FR')} · ${new Date(transaction.occurred_at).toLocaleString('fr-FR')}` })) }] : []),
      { title: 'Localisation', rows: [{ label: 'Coordonnées', value: coords ? `${coords.lat}, ${coords.lng}` : 'Non disponible' }] },
    ],
  };
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/80 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={onClose}>
    <section className="modal-sheet max-h-[92vh] w-full overflow-y-auto rounded-t-[30px] border border-white/15 bg-[#111a2b]/95 p-4 shadow-2xl sm:max-w-xl sm:rounded-[30px]" onMouseDown={(event) => event.stopPropagation()} aria-modal="true" role="dialog" aria-label={`Détails du POS ${item.pos.denomination}`}>
      <div className="modal-sticky-header"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.17em] text-cyan-200">Détail opérationnel POS</p><h2 className="mt-1 truncate text-lg font-black text-white">{item.pos.denomination}</h2><p className="mt-1 text-xs text-gray-400">{item.pos.agent_number} · {item.pos.pool} · {item.pos.address}</p></div><div className="flex shrink-0 items-center gap-2"><DetailPdfExportButton document={detailDocument}/><button type="button" onClick={onClose} className="rounded-2xl border border-white/10 bg-white/5 p-2 text-gray-300 transition hover:bg-white/10" title="Fermer"><X size={18}/></button></div></div></div>
      ...
      {item.status !== 'inactive' && (item.transactionCount > 0 ? <section className="mt-4 rounded-2xl border border-amber-300/25 bg-amber-500/[0.06] p-3"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-amber-200">Statut opérationnel verrouillé</p><p className="mt-1 text-[10px] leading-relaxed text-gray-400">Une transaction est déjà enregistrée sur ce POS. Il ne peut plus être déclaré non actif.</p></section> : <section className="mt-4 rounded-2xl border border-amber-300/30 bg-amber-500/[0.06] p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-amber-200">Correction après clôture</p><p className="mt-1 text-[10px] leading-relaxed text-gray-400">Déclarez ce POS non actif en cas d’oubli terrain. Il sera exclu des transactions requises, y compris après clôture.</p></div><button type="button" disabled={savingInactive || !item.visit || !inactiveNote.trim()} onClick={() => void markInactive()} className="shrink-0 rounded-xl border border-amber-300/35 bg-amber-500/15 px-3 py-2 text-[9px] font-black uppercase text-amber-100 transition hover:bg-amber-500/25 disabled:cursor-not-allowed disabled:opacity-45">{savingInactive ? 'Correction…' : 'Marquer non actif'}</button></div><textarea value={inactiveNote} onChange={(event) => setInactiveNote(event.target.value)} rows={2} placeholder="Constat obligatoire : pourquoi le POS est non actif ?" className="app-input mt-3 w-full resize-none rounded-xl px-3 py-2.5 text-sm"/>{inactiveError && <p className="mt-2 text-[10px] font-bold text-rose-200">{inactiveError}</p>}</section>)}
      <section className="mt-4 rounded-2xl border border-fuchsia-300/20 bg-fuchsia-500/[0.05] p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-fuchsia-200">MFS associé</p><p className="mt-0.5 text-[10px] text-gray-400">Modifiable depuis la fiche POS.</p></div><button type="button" disabled={savingMfs} onClick={() => void saveMfs()} className="rounded-xl border border-fuchsia-300/30 bg-fuchsia-500/15 px-3 py-2 text-[9px] font-black uppercase text-fuchsia-100 transition hover:bg-fuchsia-500/25 disabled:opacity-45">{savingMfs ? 'Sauvegarde…' : 'Enregistrer'}</button></div><input value={mfsName} onChange={(event) => setMfsName(event.target.value)} placeholder="Nom du MFS" className="app-input mt-3 w-full rounded-xl px-3 py-2.5 text-sm"/>{mfsError && <p className="mt-2 text-[10px] font-bold text-rose-200">{mfsError}</p>}</section>
