...
export const MerchantPosControlDetailModal: React.FC<MerchantPosControlDetailModalProps> = ({ item, transactionsPerPosTarget, onClose }) => {
  ...
  const statusLabel = item.status === 'completed' ? 'Complété' : item.status === 'active' ? 'Actif' : 'À traiter';
  const statusClass = item.status === 'completed' ? 'border-emerald-300/45 bg-emerald-500/15 text-emerald-100' : item.status === 'active' ? 'border-cyan-300/45 bg-cyan-500/15 text-cyan-100' : 'border-violet-300/35 bg-violet-500/12 text-violet-100';
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/80 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={onClose}>
    <section className="modal-sheet max-h-[92vh] w-full overflow-y-auto rounded-t-[30px] border border-white/15 bg-[#111a2b]/95 p-4 shadow-2xl sm:max-w-xl sm:rounded-[30px]" onMouseDown={(event) => event.stopPropagation()} aria-modal="true" role="dialog" aria-label={`Détails du POS ${item.pos.denomination}`}>
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[0.17em] text-cyan-200">Détail opérationnel POS</p><h2 className="mt-1 truncate text-lg font-black text-white">{item.pos.denomination}</h2><p className="mt-1 text-xs text-gray-400">{item.pos.agent_number} · {item.pos.pool} · {item.pos.address}</p></div><button type="button" onClick={onClose} className="rounded-2xl border border-white/10 bg-white/5 p-2 text-gray-300 transition hover:bg-white/10" title="Fermer"><X size={18}/></button></div>
      <div className="mt-4 flex flex-wrap gap-2"><span className={`rounded-xl border px-2.5 py-1 text-[10px] font-black uppercase ${statusClass}`}>{statusLabel}</span><span className="rounded-xl border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] font-black text-gray-200">{item.transactionCount}/{transactionsPerPosTarget} transactions</span>{item.ba && <span className="rounded-xl border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[10px] font-black text-gray-200">BA : {item.ba.name}</span>}</div>
      <section className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035]"><div className="flex items-center gap-2 border-b border-white/10 px-3 py-2"><Camera size={16} className="text-cyan-200"/><span className="text-[10px] font-black uppercase tracking-[0.14em] text-gray-200">Arrivée au POS</span></div><div className="grid grid-cols-[7rem_1fr] gap-3 p-3"><div className="flex h-24 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-black/20">{arrivalPhoto ? <img src={arrivalPhoto} alt={`Arrivée à ${item.pos.denomination}`} className="h-full w-full object-cover"/> : <Camera size={22} className="text-gray-600"/>}</div><div className="min-w-0"><p className="flex items-center gap-1 text-xs font-bold text-white"><Clock3 size={14} className="text-cyan-200"/>{arrival}</p><p className="mt-1 text-[10px] text-gray-400">{item.visit?.accuracy_m != null ? `Précision GPS : ${Math.round(item.visit.accuracy_m)} m` : 'Précision GPS non disponible'}</p><p className="mt-1 text-[10px] text-gray-400">{item.ba ? `Visite enregistrée par ${item.ba.name}` : 'POS non encore attribué à une visite'}</p></div></div></section>
      <section className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035]"><div className="flex items-center gap-2 border-b border-white/10 px-3 py-2"><ReceiptText size={16} className="text-amber-200"/><span className="text-[10px] font-black uppercase tracking-[0.14em] text-gray-200">Transactions réalisées</span></div><div className="divide-y divide-white/[0.07]">{Array.from({ length: transactionsPerPosTarget }, (_, index) => item.transactions[index]).map((transaction, index) => <div key={transaction?.id || `pending-${index}`} className="flex items-center gap-3 p-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-black/20">{transaction && proofs[transaction.id] ? <img src={proofs[transaction.id]} alt={`Preuve transaction ${index + 1}`} className="h-full w-full object-cover"/> : transaction ? <ReceiptText size={16} className="text-amber-200"/> : <span className="text-xs font-black text-gray-600">T{index + 1}</span>}</div><div className="min-w-0 flex-1">{transaction ? <><p className="truncate text-xs font-black text-white">{transaction.client_number || transaction.transaction_reference || 'Transaction sans référence'}</p><p className="mt-0.5 text-[10px] text-gray-400">{Number(transaction.amount || 0).toLocaleString('fr-FR')} · {new Date(transaction.occurred_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p></> : <p className="text-xs font-bold text-gray-500">Transaction {index + 1} en attente</p>}</div>{transaction && <CheckCircle2 size={16} className="text-emerald-300"/>}</div>)}</div></section>
