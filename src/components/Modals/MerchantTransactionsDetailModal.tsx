...
export const MerchantTransactionsDetailModal: React.FC<MerchantTransactionsDetailModalProps> = ({ isOpen, activity, initialTransaction = null, onClose }) => {
  ...
  const detailDocument = {
    title: selected ? 'Détail transaction Merchant' : 'Liste des transactions Merchant',
    subtitle: `${activity.ba.name} · ${activity.ba.phone}`,
    filename: selected ? `transaction-${selected.id}` : `transactions-${activity.ba.name}`,
    sections: selected ? [
      { title: 'Transaction', rows: [{ label: 'POS', value: selected.point_of_sale?.denomination }, { label: 'Short-code', value: selected.point_of_sale?.agent_number }, { label: 'Client', value: selected.client_number }, { label: 'Référence', value: selected.transaction_reference }, { label: 'Montant', value: amount(selected.amount) }, { label: 'Date et heure', value: new Date(selected.occurred_at).toLocaleString('fr-FR') }, { label: 'Coordonnées', value: typeof selected.latitude === 'number' && typeof selected.longitude === 'number' ? `${selected.latitude}, ${selected.longitude}` : null }] },
      ...(selected.comment ? [{ title: 'Commentaire', text: selected.comment }] : []),
    ] : [{ title: 'Transactions affichées', rows: transactions.map((transaction, index) => ({ label: `#${index + 1} · ${transaction.point_of_sale?.denomination || 'POS Merchant'}`, value: `${transaction.client_number || 'Client non renseigné'} · ${amount(transaction.amount)} · ${time(transaction.occurred_at)}` })) }],
  };
  ...
  return <div className="fixed inset-0 z-[150] flex items-end justify-center bg-black/80 p-0 backdrop-blur-md sm:items-center sm:p-4" onClick={onClose}>
    <section className="modal-sheet max-h-[92vh] w-full max-w-xl overflow-y-auto p-5 sm:rounded-3xl" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="Transactions du Brand Ambassador">
      <div className="modal-sticky-header"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="rounded-2xl bg-amber-500/15 p-3 text-amber-200">{selected ? <ReceiptText size={20}/> : <Banknote size={20}/>}</div><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-200/70">Transactions Merchant</p><h2 className="mt-1 text-lg font-black">{selected ? selected.point_of_sale?.denomination || 'Détail transaction' : `${transactions.length} transaction${transactions.length > 1 ? 's' : ''}`}</h2><p className="mt-0.5 text-[11px] text-gray-400">{activity.ba.name}</p></div></div><div className="flex shrink-0 items-center gap-2"><DetailPdfExportButton document={detailDocument}/><button type="button" onClick={selected ? () => setSelected(null) : onClose} className="rounded-xl border border-white/10 bg-white/5 p-2 text-gray-300 transition hover:bg-white/10 hover:text-white" aria-label={selected ? 'Retour à la liste' : 'Fermer'}>{selected ? <ArrowLeft size={18}/> : <X size={18}/>}</button></div></div></div>
