...
import { Banknote, CalendarDays, CheckCircle2, Download, FileSpreadsheet, FileText, MapPin, ReceiptText, Store, TrendingUp, UserRound, X } from 'lucide-react';
...
export const MerchantBAOperationsModal: React.FC<MerchantBAOperationsModalProps> = ({ isOpen, mode, activity, run, onClose }) => {
  ...
  const [selectedRecentTransaction, setSelectedRecentTransaction] = useState<Awaited<ReturnType<typeof getMerchantBAActivityDetail>>['transactions'][number] | null>(null);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/80 p-0 backdrop-blur-md sm:items-center sm:p-4" onClick={onClose}>
        ...
        {loading ? <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-center text-xs font-black uppercase tracking-widest text-gray-400">Chargement…</div> : <>
          ...
          {mode === 'report' && <div className="space-y-3">
            {!attendance ? <div className="rounded-2xl border border-red-400/25 bg-red-500/10 p-4 text-sm text-red-100">Aucun rapport quotidien n’est disponible pour cette date.</div> : <>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-gray-500">Commentaire de clôture</p><p className="mt-2 text-sm text-gray-200">{cleanReportComment(attendance.closing_comment) || 'Aucun commentaire renseigné.'}</p></div>
              <div className="flex flex-wrap justify-end gap-2"><button type="button" onClick={() => void exportMerchantAgentReportExcel({ ba: activity.ba, attendance, visits, transactions, run })} className="inline-flex items-center gap-2 rounded-xl border border-emerald-300/30 bg-emerald-500/15 px-3 py-2 text-[10px] font-black uppercase text-emerald-100 transition hover:bg-emerald-500/25"><FileSpreadsheet size={14}/>Excel</button><button type="button" onClick={() => void exportMerchantAgentReportPdf({ ba: activity.ba, attendance, visits, transactions, run })} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300/30 bg-cyan-500/15 px-3 py-2 text-[10px] font-black uppercase text-cyan-100 transition hover:bg-cyan-500/25"><Download size={14}/>PDF</button></div>
            </>}
          </div>}
      ...
      <MerchantTransactionsDetailModal isOpen={showTransactions || Boolean(selectedRecentTransaction)} activity={activity} initialTransaction={selectedRecentTransaction} onClose={() => { setShowTransactions(false); setSelectedRecentTransaction(null); }}/>
