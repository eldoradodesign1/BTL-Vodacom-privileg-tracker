      ...
      <div className="modal-sheet relative w-full max-w-lg max-h-[92vh] overflow-y-auto" onClick={(event) => event.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={password} onChange={(event) => setPassword(event.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-white text-sm focus:outline-none focus:border-red-500" />
          </div>
            </>
          )}

          {error && <div className="text-xs text-red-400 font-bold bg-red-500/10 border border-red-500/20 rounded-xl p-3">{error}</div>}
          <button type="submit" disabled={saving} className="w-full bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-black uppercase text-xs rounded-2xl py-4 transition-colors">
            {saving ? 'Enregistrement…' : mode === 'assign' ? 'Enregistrer les affectations' : 'Créer l’utilisateur'}
          </button>
        </form>
      </div>
    </div>
