...
export const MerchantFundRequestModal: React.FC<MerchantFundRequestModalProps> = ({ isOpen, currentUser, run, positions, visits, mfsName, onClose, onSubmitted }) => {
  ...
  const filteredPositions = useMemo(() => {
    ...
    return source.filter((pos) => !needle || `${pos.denomination} ${pos.agent_number} ${pos.pool}`.toLowerCase().includes(needle));
  }, [mfsName, positions, query]);
  ...
  useEffect(() => {
    ...
    setError('');
  }, [isOpen, visits, positions, mfsName]);
