...
import type { BAPosVisit, BATransaction, CampaignRun, PointOfSale, User } from '../../types';
...
export const MerchantTransactionModal: React.FC<MerchantTransactionModalProps> = ({
  isOpen, currentUser, run, positions, visits, activityDate, mfsName, onClose, onRecorded, onPosArrivalRecorded,
}) => {
  ...
  const [arrivalQueued, setArrivalQueued] = useState(false);
  ...
  const resetForm = () => {
    ...
    setOcrState('idle'); setCreatedVisit(null); setArrivalQueued(false); setError(''); setIsComplete(false); setLightbox(null);
  };
  ...
  const choosePos = (pos: PointOfSale) => {
    if (inactivePosIds.has(pos.id)) {
      setError('Ce POS est déclaré non actif. Réactivez-le depuis Mes POS avant d’y saisir une transaction.');
      return;
    }
  ...
  const saveArrival = () => {
    ...
    if (isInactivePos) return setError('Ce POS est déclaré non actif. Réactivez-le depuis Mes POS avant de poursuivre.');
    if (!arrivalPhoto) return setError('Ajoutez la photo d’arrivée du POS.');
    const activeRun = run;
    const pos = selectedPos;
    const photo = arrivalPhoto;
    setArrivalQueued(true);
    runInBackground('Arrivée au POS', async () => {
      const { geo } = await resolveGeoForPatience();
      const path = await uploadMerchantEvidence(MERCHANT_CAMPAIGN_CODE, `${currentUser.id}/${activityDate}/pos-arrival-${pos.id}-${Date.now()}.jpg`, photo);
      return recordPosArrival({ daily_assignment_id: null, campaign_run_id: activeRun.id, ba_id: currentUser.id, pos_id: pos.id, activity_date: activityDate, visited_at: new Date().toISOString(), latitude: geo.latitude, longitude: geo.longitude, accuracy_m: geo.accuracy, arrival_photo_path: path, status: 'visited', comment: null });
    }, {
      queued: 'Arrivée POS lancée en arrière-plan.',
      success: 'Arrivée POS synchronisée.',
      onSuccess: (visit) => { setArrivalQueued(false); setCreatedVisit(visit); onPosArrivalRecorded(visit); },
      onError: (caught) => { setArrivalQueued(false); setError(caught.message); },
    });
  };
  ...
  const save = () => {
    ...
    if (isInactivePos || posVisit.operational_status === 'inactive') return setError('Ce POS est déclaré non actif. Aucune transaction ne peut y être saisie tant qu’il n’est pas réactivé.');
    ...
    const photo = transactionPhoto;
    const transactionReference = hasUnreadableReference ? null : reference.trim() || null;
    const formattedClientNumber = formatMsisdn(cleanPhoneNumber(clientNumber));
    const transactionComment = comment.trim() || null;
    setIsComplete(true);
    runInBackground('Transaction', async () => {
      const { geo } = await resolveGeoForPatience();
      const path = await uploadMerchantEvidence(MERCHANT_CAMPAIGN_CODE, `${currentUser.id}/${activityDate}/transaction-${Date.now()}.jpg`, photo);
      return createTransaction({ campaign_run_id: activeRun.id, ba_id: currentUser.id, pos_id: pos.id, pos_visit_id: visit.id, transaction_reference: transactionReference, client_number: formattedClientNumber, amount: numericAmount, evidence_path: path, occurred_at: new Date().toISOString(), latitude: geo.latitude, longitude: geo.longitude, accuracy_m: geo.accuracy, comment: transactionComment, status: 'recorded' }, Number(activeRun.transactions_per_pos_target || 3));
    }, {
      queued: 'Transaction lancée en arrière-plan.',
      success: 'Transaction synchronisée.',
      onSuccess: (transaction) => onRecorded(transaction),
