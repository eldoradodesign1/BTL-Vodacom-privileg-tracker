...
type OcrState = 'idle' | 'scanning' | 'found' | 'unreadable' | 'unavailable';
...
export const MerchantTransactionModal: React.FC<MerchantTransactionModalProps> = ({
  ...
  const hasUnreadableReference = reference === 'N° transaction illisible' || ocrState === 'unreadable';
  ...
  const chooseTransactionPhoto = (file: File | null) => {
    ...
    void identifyTransactionReference(file).then((result) => {
      if (!result.available) { setOcrState('unavailable'); return; }
      if (result.transactionId) { setReference(result.transactionId); setOcrState('found'); }
      else { setReference('N° transaction illisible'); setOcrState('unreadable'); }
    }).catch(() => { setReference('N° transaction illisible'); setOcrState('unreadable'); });
  };
