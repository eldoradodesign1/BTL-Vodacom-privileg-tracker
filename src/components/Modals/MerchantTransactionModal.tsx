...
import { Camera, CheckCircle2, Command, Save, X } from 'lucide-react';
import type { CampaignRun, PointOfSale, User } from '../../types';
import { createTransaction, MERCHANT_CAMPAIGN_CODE, uploadMerchantEvidence } from '../../utils/merchantCampaign';
...
export const MerchantTransactionModal: React.FC<MerchantTransactionModalProps> = ({
  ...
  const save = async () => {
    ...
    if (!selectedPos) {
      setError('Recherchez et sélectionnez un POS.');
      return;
    }
      ...
      <section className="modal-sheet max-h-[92vh] w-full max-w-xl overflow-y-auto p-5 sm:rounded-3xl">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-cyan-500/15 p-3 text-cyan-200"><Command size={21} /></div>
            <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200/70">Merchant</p><h2 id="merchant-transaction-title" className="mt-1 text-lg font-black">Nouvelle transaction</h2></div>
          </div>
        ...
        {isComplete ? (
          <div className="py-10 text-center">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500/15 text-emerald-300"><CheckCircle2 size={30} /></div>
            <h3 className="mt-4 text-lg font-black">Transaction enregistrée</h3>
            <p className="mt-2 text-sm text-gray-400">La capture, le POS, le client, le montant et la position GPS ont été sauvegardés.</p>
            <div className="mt-6 grid grid-cols-2 gap-3"><button type="button" onClick={resetForm} className="rounded-2xl border border-cyan-300/30 bg-cyan-400/[0.07] px-4 py-3 text-xs font-black uppercase text-cyan-100">Nouvelle transaction</button><button type="button" onClick={onClose} className="btn-neon btn-red px-4 py-3 text-xs">Terminé</button></div>
          </div>
        ) : (
          <div className="mt-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2"><input value={amount} onChange={(event) => setAmount(event.target.value)} disabled={saving} inputMode="decimal" placeholder="Montant" className="app-input rounded-2xl px-4 py-3 text-sm"/><input value={clientNumber} onChange={(event) => setClientNumber(event.target.value)} disabled={saving} inputMode="tel" placeholder="N° client" className="app-input rounded-2xl px-4 py-3 text-sm"/></div>
            <input value={reference} onChange={(event) => setReference(event.target.value)} disabled={saving} placeholder="N° transaction (optionnel)" className="app-input w-full rounded-2xl px-4 py-3 text-sm"/>
            <input value={comment} onChange={(event) => setComment(event.target.value)} disabled={saving} placeholder="Commentaire (optionnel)" className="app-input w-full rounded-2xl px-4 py-3 text-sm"/>
            <input ref={inputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(event) => { setTransactionPhoto(event.target.files?.[0] || null); event.currentTarget.value = ''; }} />
            <button type="button" onClick={() => inputRef.current?.click()} disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/15 bg-white/5 px-4 py-3 text-xs font-black uppercase transition hover:bg-white/10 disabled:opacity-40"><Camera size={16}/>{transactionPhoto ? 'Capture confirmée — prête à enregistrer' : 'Ajouter la capture de transaction'}</button>
            {transactionPhoto && <p className="rounded-2xl border border-emerald-400/25 bg-emerald-500/[0.08] px-3 py-2 text-center text-[11px] font-bold text-emerald-200">{transactionPhoto.name}</p>}
            <button type="button" disabled={saving || !transactionPhoto} onClick={save} className="btn-neon btn-red flex w-full items-center justify-center gap-2 disabled:opacity-40"><Save size={16}/><span>{saving ? 'Enregistrement…' : 'Enregistrer la transaction'}</span></button>
          </div>
        )}
      </section>
