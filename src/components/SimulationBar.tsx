...
export const SimulationBar: React.FC<SimulationBarProps> = ({
    ...
    return (
      <>
        <button
          ...
          title={`Simulation · ${selectedUser.name}`}
          aria-label={`Ouvrir la simulation · ${selectedUser.name}`}
          className={`fixed right-4 top-4 z-[900] flex h-11 w-11 items-center justify-center rounded-2xl border backdrop-blur-2xl shadow-2xl transition-all duration-300 hover:scale-110 hover:-rotate-3 active:scale-95 ${surface}`}
        >
        ...
        {pickerOpen && typeof document !== 'undefined' && createPortal(( 
          <div className="fixed inset-0 z-[1000] flex items-start justify-center bg-black/55 p-3 pt-[max(4rem,12vh)] backdrop-blur-sm" onPointerDown={() => { setPickerOpen(false); setSelectorQuery(''); }}>
            <div className={`w-full max-w-md overflow-hidden rounded-[1.7rem] border shadow-[0_30px_100px_rgba(0,0,0,.55)] ${surface}`} onPointerDown={(event) => event.stopPropagation()}>
              <div className="flex items-center gap-2 border-b border-white/10 p-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-red-500/10 text-red-200"><UserRound size={16}/></div><div className="min-w-0 flex-1"><p className="text-[8px] font-black uppercase tracking-[0.2em] text-red-200/80">Simulation Master</p><p className="truncate text-sm font-black">{selectedUser.name}</p></div><button type="button" onClick={() => { setPickerOpen(false); setSelectorQuery(''); }} className={`rounded-xl p-2 ${muted}`}><X size={16}/></button></div>
              <div className="p-3"><div className={`flex items-center gap-2 rounded-xl border px-3 py-2.5 ${chip}`}><Search size={14} className={muted}/><input autoFocus value={selectorQuery} onChange={(event) => setSelectorQuery(event.target.value)} placeholder="Rechercher un utilisateur…" className={`min-w-0 flex-1 bg-transparent text-[10px] font-bold outline-none ${isDiamondTheme ? 'placeholder:text-slate-400' : 'placeholder:text-white/30'}`}/></div>
                <div className="mt-2 max-h-[55vh] space-y-1 overflow-y-auto pr-1 custom-scrollbar">{filteredUsers.map((user) => <button key={user.id} type="button" onClick={() => { onSimulateUserChange(user.id); setPickerOpen(false); setSelectorQuery(''); }} className={`flex w-full items-center gap-2 rounded-xl px-2.5 py-2.5 text-left transition ${user.id === effectiveUser.id ? (isDiamondTheme ? 'bg-slate-800 text-white' : 'bg-red-500/20 text-white') : (isDiamondTheme ? 'text-slate-700 hover:bg-slate-100' : 'text-gray-300 hover:bg-white/10')}`}><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white/10"><UserRound size={13}/></span><span className="min-w-0 flex-1"><b className="block truncate text-[10px]">{user.name}</b><span className="text-[8px] font-black uppercase opacity-50">{user.role} · {user.userCategory || '—'}</span></span></button>)}</div>
              </div>
            </div>
          </div>
        ), document.body)}
  ...
  return (
    <>
      <div className={`sticky top-0 z-[70] border-b backdrop-blur-2xl ${surface} animate-pop`}>
        <div className="mx-auto flex min-h-11 max-w-6xl items-center gap-2 px-2.5 py-1.5 sm:px-4">
          <div className="flex min-w-0 items-center gap-2"><div className="relative flex h-7 w-7 shrink-0 items-center justify-center rounded-xl border border-cyan-300/20 bg-cyan-400/10 text-cyan-100"><Shield size={14}/><span className="absolute -right-0.5 -top-0.5 h-1.5 w-1.5 rounded-full bg-cyan-300 shadow-[0_0_8px_currentColor]"/></div><div className="hidden min-w-0 sm:block"><p className="text-[8px] font-black uppercase tracking-[0.2em] text-cyan-200/80">Simulation Master</p><p className={`truncate text-[9px] font-bold ${muted}`}>Origine · {masterUser.name}</p></div></div>
          <button type="button" onClick={() => setPickerOpen(true)} className={`group flex min-w-0 flex-1 items-center gap-2 rounded-xl border px-2 py-1.5 text-left transition-all duration-300 hover:border-red-300/40 ${chip}`} title="Choisir l'utilisateur simulé"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-white/10"><UserRound size={12}/></span><span className="min-w-0 flex-1"><b className="block truncate text-[9px] font-black">{selectedUser.name}</b><span className={`block truncate text-[7px] font-black uppercase tracking-wider ${muted}`}>{selectedUser.role} · {selectedUser.userCategory || 'utilisateur'}</span></span><SlidersHorizontal size={13} className={`shrink-0 group-hover:text-red-300 ${muted}`}/></button>
          <div className={`hidden items-center gap-1 rounded-xl border p-1 sm:flex ${chip}`}>{(['agent','supervisor','admin'] as UserRole[]).map((role) => { const target=shortcutTargets[role as keyof typeof shortcutTargets]; const label=role==='agent'?'AGENT':role==='supervisor'?'SUP':'ADMIN'; const name=target?.name || (role==='agent'?'Agent Test':role==='supervisor'?'Hervé Ntalu':'Bradley Izamaboko'); return <button key={role} type="button" onClick={() => onSimulateRole(role)} className={`rounded-lg px-2.5 py-1 text-[8px] font-black uppercase transition ${activeRole===role?activeChip:muted+' hover:text-white'}`} title={`Simuler ${name}`}>{label} · <span className="normal-case tracking-normal">{name}</span></button>; })}</div>
          ...
          <button type="button" onClick={() => setExpanded(false)} className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border transition ${chip} ${muted}`} title="Réduire en icône flottante" aria-label="Réduire en icône flottante"><Minimize2 size={13}/></button>
        </div>
      </div>
      {pickerOpen && typeof document !== 'undefined' && createPortal(( 
        <div className="fixed inset-0 z-[1000] flex items-start justify-center bg-black/55 p-3 pt-[max(4rem,12vh)] backdrop-blur-sm" onPointerDown={() => { setPickerOpen(false); setSelectorQuery(''); }}>
