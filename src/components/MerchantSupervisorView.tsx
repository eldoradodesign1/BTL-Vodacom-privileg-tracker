...
import { getActiveCampaignRuns, getMerchantCampaign, getMerchantMonitoring, getMerchantPosControl, merchantTodayIso, updateMerchantTargetSettings, type MerchantPosControlItem, type MerchantTeamActivity } from '../utils/merchantCampaign';
...
export const MerchantSupervisorView: React.FC<MerchantSupervisorViewProps> = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [nextTeam, nextControls] = await Promise.all([getMerchantMonitoring(activeRun.id, merchantTodayIso()), getMerchantPosControl(activeRun)]);
