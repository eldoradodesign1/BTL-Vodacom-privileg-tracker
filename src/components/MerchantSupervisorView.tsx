...
import { CalendarDays, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, CircleDot, FileText, Filter, MapPin, PauseCircle, PlayCircle, Plus, Save, Search, Target, UserRound, UsersRound } from 'lucide-react';
import type { CampaignPause, CampaignRun, User } from '../types';
import { createCampaignPause, deleteCampaignPause, endCampaignPause, getActiveCampaignRuns, getCampaignPauses, getMerchantCampaign, getMerchantMonitoring, getMerchantPosControl, isCampaignPausedOn, MERCHANT_CAMPAIGN_START, merchantTodayIso, updateMerchantTargetSettings, type MerchantPosControlItem, type MerchantTeamActivity } from '../utils/merchantCampaign';
...
export const MerchantSupervisorView: React.FC<MerchantSupervisorViewProps> = ({ currentUser }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [nextTeam, nextControls, nextPauses] = await Promise.all([getMerchantMonitoring(activeRun.id, merchantTodayIso()), getMerchantPosControl(activeRun), getCampaignPauses(nextCampaign.id)]);
      setRun(activeRun); setTeam(nextTeam); setControls(nextControls); setPauses(nextPauses);
      setTargets({
  ...
  return <div className="space-y-4 pb-4">
    ...
    <nav aria-label="Sections Gestion Merchant" className="glass-card grid grid-cols-4 gap-1.5 p-1.5">{([{ id: 'ba', label: 'BA', Icon: UsersRound }, { id: 'pos', label: 'POS', Icon: Filter }, { id: 'mfs', label: 'MFS', Icon: Target }, { id: 'targets', label: 'Targets', Icon: Save }] as const).map(({ id, label, Icon }) => <button key={id} type="button" onClick={() => setManagementTab(id)} className={`flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[9px] font-black uppercase transition sm:flex-row sm:px-3 ${managementTab === id ? 'bg-violet-500/25 text-violet-100 shadow-[0_8px_18px_rgba(139,92,246,0.14)]' : 'text-gray-400 hover:bg-white/[0.05] hover:text-white'}`}><Icon size={14}/><span className="truncate">{label}</span></button>)}</nav>
