...
export const MerchantSupervisorView: React.FC<MerchantSupervisorViewProps> = ({ currentUser, openFundRequestId, onFundRequestOpened, openFundRequests = false, onFundRequestsOpened }) => {
  ...
  return <div className="space-y-4 pb-4">
    ...
    <MerchantFundRequestsModal isOpen={isFundRequestsOpen} requests={fundRequests} onClose={() => setIsFundRequestsOpen(false)} onOpenReport={() => setIsFundRequestReportOpen(true)} onSelect={(request) => { setIsFundRequestsOpen(false); setSelectedFundRequest(request); }} />
    {isFundRequestReportOpen && <React.Suspense fallback={null}><MerchantFundRequestsReportModal isOpen requests={fundRequests} onClose={() => setIsFundRequestReportOpen(false)} /></React.Suspense>}
