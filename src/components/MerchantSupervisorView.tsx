...
export const MerchantSupervisorView: React.FC<MerchantSupervisorViewProps> = ({ currentUser, openFundRequestId, onFundRequestOpened, openFundRequests = false, onFundRequestsOpened }) => {
  ...
  const [managementTab, setManagementTab] = useState<'ba' | 'pos' | 'tx' | 'mfs' | 'targets'>('ba');
  ...
  const [allTransactions, setAllTransactions] = useState<MerchantTransactionWithBa[]>([]);
  const [txLoading, setTxLoading] = useState(false);
  const [txLoadedRunId, setTxLoadedRunId] = useState<string | null>(null);
  const [txQuery, setTxQuery] = useState('');
  const [txStatus, setTxStatus] = useState<'all' | MerchantTransactionWithBa['status']>('all');
  const [txPeriod, setTxPeriod] = useState({ startDate: MERCHANT_CAMPAIGN_START, endDate: merchantTodayIso() });
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return allTransactions.filter((transaction) => {
      const activityDate = transaction.occurred_at.slice(0, 10);
      const matchesDate = activityDate >= txPeriod.startDate && activityDate <= txPeriod.endDate;
      const matchesStatus = txStatus === 'all' || transaction.status === txStatus;
      const haystack = `${transaction.ba?.name || ''} ${transaction.ba?.phone || ''} ${transaction.point_of_sale?.agent_number || ''} ${transaction.point_of_sale?.denomination || ''} ${transaction.point_of_sale?.pool || ''} ${transaction.point_of_sale?.mfs_name || ''} ${transaction.client_number || ''} ${transaction.transaction_reference || ''}`.toLowerCase();
      return matchesDate && matchesStatus && (!needle || haystack.includes(needle));
    });
  }, [allTransactions, txPeriod, txQuery, txStatus]);
  const totalTxPages = Math.max(1, Math.ceil(filteredTransactions.length / POS_PAGE_SIZE));
  const safeTxPage = Math.min(txPage, totalTxPages);
  const paginatedTransactions = filteredTransactions.slice((safeTxPage - 1) * POS_PAGE_SIZE, safeTxPage * POS_PAGE_SIZE);
  ...
  const archiveRejectedFundRequests = async (): Promise<number> => {
    if (currentUser.role !== 'super_admin') throw new Error('Seul le superadmin peut archiver les demandes rejetées.');
    const count = await archiveRejectedMerchantFundRequests(currentUser.id);
    setFundRequests((current) => current.filter((request) => request.status !== 'rejected'));
    setFundRequestReportRequests((current) => current.filter((request) => request.status !== 'rejected'));
    return count;
  };
  ...
  return <div className="space-y-4 pb-4">
    ...
    {managementTab === 'pos' && <section className="merchant-inventory-shell glass-card space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><Filter className="shrink-0 text-rose-200" size={19}/><div><h2 className="font-black">Inventaire et suivi des POS</h2><p className="text-xs text-gray-400">Un POS atteint {targets.transactions_per_pos_target} transactions pour être complété ; s’il est quitté ou clôturé avant, il apparaît Inachevé.</p></div></div><div className="flex shrink-0 flex-col items-end gap-1.5"><button type="button" onClick={() => setIsCreatePosOpen(true)} className="inline-flex items-center gap-1.5 rounded-xl border border-cyan-300/35 bg-cyan-400/10 px-3 py-2 text-[10px] font-black uppercase text-cyan-100 transition hover:bg-cyan-400/20"><Plus size={14}/>Nouveau POS</button><button type="button" onClick={() => setIsInventoryExportOpen(true)} title="Exporter les POS et MFS" aria-label="Exporter les POS et MFS" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-cyan-300/25 bg-cyan-500/[0.08] text-cyan-100 transition hover:border-cyan-200/50 hover:bg-cyan-500/[0.16]"><Download size={14}/></button></div></div><div className="flex items-center justify-between rounded-xl border border-white/[0.08] bg-black/15 px-3 py-2"><span className="text-[10px] font-bold text-gray-400">POS à faire</span><b className="text-sm font-black text-violet-100">{inventoryCounts.statuses.pending}</b></div><div className="grid grid-cols-3 gap-2 text-center">{([{ id: 'inactive', label: 'Inactifs', value: inventoryCounts.statuses.inactive, className: 'border-amber-300/25 bg-amber-500/[0.08] text-amber-100' }, { id: 'incomplete', label: 'Inachevés', value: inventoryCounts.statuses.incomplete, className: 'border-rose-300/25 bg-rose-500/[0.08] text-rose-100' }, { id: 'completed', label: 'Complétés', value: inventoryCounts.statuses.completed, className: 'border-emerald-300/25 bg-emerald-500/[0.08] text-emerald-100' }] as const).map((stat) => <button key={stat.id} type="button" onClick={() => setStateFilter(stat.id)} className={`rounded-2xl border p-2 transition ${stat.className} ${stateFilter === stat.id ? 'ring-1 ring-white/70' : 'opacity-80 hover:opacity-100'}`}><b className="block text-base font-black">{stat.value}</b><span className="text-[8px] font-black uppercase">{stat.label}</span></button>)}</div><div className="relative"><Search className="absolute left-3 top-3.5 text-gray-500" size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Short code, nom, adresse, MFS ou BA" className="app-input w-full rounded-2xl py-3 pl-10 pr-4 text-sm"/></div><div className="flex gap-2 overflow-x-auto pb-1"><button type="button" onClick={() => setStateFilter('all')} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-[10px] font-black uppercase ${stateFilter === 'all' ? 'border-rose-300/50 bg-rose-500/20 text-rose-100' : 'border-white/10 bg-white/5 text-gray-300'}`}>Tous · {controls.length}</button>{POOLS.map((item) => <button type="button" key={item} onClick={() => setPool(item)} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-[10px] font-black uppercase ${pool === item ? 'border-rose-300/50 bg-rose-500/20 text-rose-100' : 'border-white/10 bg-white/5 text-gray-300'}`}>{item}{item !== 'Tous' ? ` · ${inventoryCounts.pools[item] || 0}` : ''}</button>)}</div><div className="max-h-[36rem] space-y-2 overflow-y-auto pr-1">{paginatedPos.map((item) => { const meta = statusMeta(item); const StatusIcon = meta.icon; return <button type="button" key={item.pos.id} onClick={() => setSelectedPos(item)} className="group w-full rounded-2xl border border-white/10 bg-white/[0.025] p-3 text-left transition hover:border-rose-300/35 hover:bg-white/[0.055]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><b className="block truncate text-sm text-white">{item.pos.denomination}</b><p className="mt-0.5 text-[10px] font-black uppercase text-rose-200">{item.pos.agent_number} · {item.pos.pool}</p><p className="mt-1 line-clamp-1 text-[11px] text-gray-400">{item.pos.address}</p><p className="mt-1 line-clamp-1 text-[10px] font-bold text-fuchsia-200">MFS · {item.pos.mfs_name?.trim() || 'Non renseigné'}</p>{item.ba && <p className={`mt-1 text-[10px] font-bold ${item.status === 'completed' ? 'text-emerald-200' : item.status === 'inactive' ? 'text-amber-200' : 'text-cyan-200'}`}>{item.status === 'completed' ? 'Complété par ' : item.status === 'inactive' ? 'Non actif · constaté par ' : item.status === 'incomplete' ? 'Inachevé par ' : 'Actif avec '}{item.ba.name}</p>}</div><div className="flex shrink-0 flex-col items-end gap-2"><span className={`inline-flex items-center gap-1 rounded-xl border px-2 py-1 text-[9px] font-black uppercase ${meta.className}`}><StatusIcon size={12}/>{meta.label}</span><span className="rounded-lg bg-black/20 px-2 py-1 text-[10px] font-black text-gray-200">{item.status === 'inactive' ? 'Couvert' : `${item.transactionCount}/${targets.transactions_per_pos_target} Tx`}</span></div></div></button>; })}</div>{filtered.length > 0 && <div className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/15 px-3 py-2"><p className="text-[10px] font-bold text-gray-400">{(safePosPage - 1) * POS_PAGE_SIZE + 1}–{Math.min(safePosPage * POS_PAGE_SIZE, filtered.length)} sur {filtered.length} POS</p><div className="flex items-center gap-1"><button type="button" onClick={() => setPosPage((page) => Math.max(1, page - 1))} disabled={safePosPage === 1} aria-label="Page précédente" className="rounded-lg border border-white/10 p-1.5 text-gray-300 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-35"><ChevronLeft size={14}/></button><span className="min-w-14 text-center text-[10px] font-black text-gray-200">{safePosPage}/{totalPosPages}</span><button type="button" onClick={() => setPosPage((page) => Math.min(totalPosPages, page + 1))} disabled={safePosPage === totalPosPages} aria-label="Page suivante" className="rounded-lg border border-white/10 p-1.5 text-gray-300 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-35"><ChevronRight size={14}/></button></div></div>}</section>}
    {managementTab === 'tx' && <section className="glass-card space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><ReceiptText className="shrink-0 text-amber-200" size={19}/><div><h2 className="font-black">Registre des transactions</h2><p className="text-xs text-gray-400">Toutes les transactions Merchant · détail, filtre et analyse partageable.</p></div></div><button type="button" disabled={txLoading} onClick={() => setIsTransactionsReportOpen(true)} title="Ouvrir le rapport des transactions" className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-amber-300/25 bg-amber-500/[0.1] text-amber-100 transition hover:bg-amber-500/[0.18] disabled:opacity-45"><FileText size={16}/></button></div><DateRangeKnobSlider minDate={MERCHANT_CAMPAIGN_START} maxDate={merchantTodayIso()} startDate={txPeriod.startDate} endDate={txPeriod.endDate} onChange={setTxPeriod}/><div className="flex gap-2 overflow-x-auto pb-1">{(['all', 'recorded', 'verified', 'rejected'] as const).map((status) => <button type="button" key={status} onClick={() => setTxStatus(status)} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-[10px] font-black uppercase transition ${txStatus === status ? 'border-amber-300/45 bg-amber-500/20 text-amber-100' : 'border-white/10 bg-white/[0.04] text-gray-400 hover:bg-white/[0.08]'}`}>{status === 'all' ? `Toutes · ${filteredTransactions.length}` : status === 'recorded' ? 'Enregistrées' : status === 'verified' ? 'Vérifiées' : 'Rejetées'}</button>)}</div><div className="relative"><Search className="absolute left-3 top-3.5 text-gray-500" size={16}/><input value={txQuery} onChange={(event) => setTxQuery(event.target.value)} placeholder="BA, POS, short-code, MFS, client ou référence" className="app-input w-full rounded-2xl py-3 pl-10 pr-4 text-sm"/></div>{txLoading ? <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-center text-xs font-black uppercase tracking-widest text-gray-400">Chargement du registre Tx…</div> : <><div className="max-h-[36rem] space-y-2 overflow-y-auto pr-1">{paginatedTransactions.map((transaction) => <button type="button" key={transaction.id} onClick={() => setSelectedTransaction(transaction)} className="group w-full rounded-2xl border border-white/10 bg-white/[0.025] p-3 text-left transition hover:border-amber-300/35 hover:bg-amber-500/[0.055]"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><b className="block truncate text-sm text-white">{transaction.point_of_sale?.denomination || 'POS Merchant'}</b><p className="mt-0.5 text-[10px] font-black uppercase text-amber-200">{transaction.point_of_sale?.agent_number || '—'} · {transaction.point_of_sale?.pool || 'Pool non renseigné'}</p><p className="mt-1 truncate text-[11px] text-gray-300">{transaction.ba?.name || 'BA non renseigné'} · {transaction.point_of_sale?.mfs_name || 'MFS non renseigné'}</p><p className="mt-1 truncate text-[10px] text-gray-500">Client {transaction.client_number || '—'} · Réf. {transaction.transaction_reference || 'Non renseignée'} · {new Date(transaction.occurred_at).toLocaleDateString('fr-FR')}</p></div><div className="flex shrink-0 flex-col items-end gap-2"><b className="text-sm text-emerald-100">{Number(transaction.amount || 0).toLocaleString('fr-FR')} $</b><span className={`rounded-lg border px-2 py-1 text-[9px] font-black uppercase ${transaction.status === 'verified' ? 'border-emerald-300/25 bg-emerald-500/[0.1] text-emerald-100' : transaction.status === 'rejected' ? 'border-rose-300/25 bg-rose-500/[0.1] text-rose-100' : 'border-amber-300/25 bg-amber-500/[0.1] text-amber-100'}`}>{transaction.status === 'verified' ? 'Vérifiée' : transaction.status === 'rejected' ? 'Rejetée' : 'Enregistrée'}</span></div></div></button>)}{filteredTransactions.length === 0 && <p className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-center text-xs text-gray-500">Aucune transaction ne correspond à cette période ou à ces filtres.</p>}</div>{filteredTransactions.length > 0 && <div className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-black/15 px-3 py-2"><p className="text-[10px] font-bold text-gray-400">{(safeTxPage - 1) * POS_PAGE_SIZE + 1}–{Math.min(safeTxPage * POS_PAGE_SIZE, filteredTransactions.length)} sur {filteredTransactions.length} Tx</p><div className="flex items-center gap-1"><button type="button" onClick={() => setTxPage((page) => Math.max(1, page - 1))} disabled={safeTxPage === 1} aria-label="Page précédente" className="rounded-lg border border-white/10 p-1.5 text-gray-300 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-35"><ChevronLeft size={14}/></button><span className="min-w-14 text-center text-[10px] font-black text-gray-200">{safeTxPage}/{totalTxPages}</span><button type="button" onClick={() => setTxPage((page) => Math.min(totalTxPages, page + 1))} disabled={safeTxPage === totalTxPages} aria-label="Page suivante" className="rounded-lg border border-white/10 p-1.5 text-gray-300 transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-35"><ChevronRight size={14}/></button></div></div>}</>}</section>}
    {managementTab === 'mfs' && <section className="glass-card space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="font-black">Répertoire MFS</h2><p className="mt-1 text-xs text-gray-400">Recherchez un MFS, un BA ou un POS puis ouvrez son inventaire associé.</p></div><div className="flex shrink-0 flex-col items-end gap-1.5"><span className="rounded-xl border border-fuchsia-300/25 bg-fuchsia-500/[0.08] px-2.5 py-1.5 text-[10px] font-black text-fuchsia-100">{filteredMfsGroups.length}/{mfsGroups.length}</span><button type="button" onClick={() => setIsInventoryExportOpen(true)} title="Exporter les POS et MFS" aria-label="Exporter les POS et MFS" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-fuchsia-300/25 bg-fuchsia-500/[0.08] text-fuchsia-100 transition hover:border-fuchsia-200/50 hover:bg-fuchsia-500/[0.16]"><Download size={14}/></button></div></div><div className="relative"><Search className="absolute left-3 top-3.5 text-gray-500" size={16}/><input value={mfsQuery} onChange={(event) => setMfsQuery(event.target.value)} placeholder="Rechercher MFS, BA ou POS" className="app-input w-full rounded-2xl py-3 pl-10 pr-4 text-sm"/></div><div className="flex gap-2 overflow-x-auto pb-1"><button type="button" onClick={() => setMfsBaFilter('all')} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-[10px] font-black uppercase ${mfsBaFilter === 'all' ? 'border-fuchsia-300/50 bg-fuchsia-500/20 text-fuchsia-100' : 'border-white/10 bg-white/5 text-gray-400'}`}>Tous les BA</button>{mfsBaOptions.map((ba) => <button key={ba} type="button" onClick={() => setMfsBaFilter(ba)} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-[10px] font-black uppercase ${mfsBaFilter === ba ? 'border-fuchsia-300/50 bg-fuchsia-500/20 text-fuchsia-100' : 'border-white/10 bg-white/5 text-gray-400'}`}>{ba}</button>)}</div><div className="space-y-2">{filteredMfsGroups.map(([mfs, items]) => { const baNames = Array.from(new Set(items.map((item) => item.ba?.name?.trim()).filter(Boolean) as string[])); return <button key={mfs} type="button" onClick={() => { setQuery(mfs === 'Non renseigné' ? '' : mfs); setManagementTab('pos'); }} className="flex w-full items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-left transition hover:border-fuchsia-300/35 hover:bg-fuchsia-500/[0.06]"><div className="min-w-0"><b className="block truncate text-sm text-white">{mfs}</b><p className="mt-1 text-[10px] font-bold text-cyan-100">BA · {baNames.length ? baNames.join(' · ') : 'Non renseigné'}</p><p className="mt-1 line-clamp-1 text-[10px] text-gray-400">{items.slice(0, 3).map((item) => item.pos.denomination).join(' · ')}{items.length > 3 ? '…' : ''}</p></div><span className="shrink-0 rounded-xl border border-fuchsia-300/20 bg-black/15 px-2 py-1 text-[10px] font-black text-fuchsia-100">{items.length} POS</span></button>; })}{filteredMfsGroups.length === 0 && <p className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-center text-xs text-gray-500">Aucun MFS ne correspond à cette recherche.</p>}</div></section>}
    </div>
    <MerchantFundRequestsModal isOpen={isFundRequestsOpen} requests={fundRequests} canArchiveRejected={currentUser.role === 'super_admin'} onArchiveRejected={archiveRejectedFundRequests} onClose={() => setIsFundRequestsOpen(false)} onOpenReport={(requests) => { setFundRequestReportRequests(requests); setIsFundRequestReportOpen(true); }} onSelect={(request) => { setIsFundRequestsOpen(false); setSelectedFundRequest(request); }} />
    ...
    {isTransactionsReportOpen && <React.Suspense fallback={null}><MerchantTransactionsReportModal isOpen records={filteredTransactions} startsOn={txPeriod.startDate} endsOn={txPeriod.endDate} generatedBy={currentUser.name} onClose={() => setIsTransactionsReportOpen(false)} /></React.Suspense>}
    {selectedTransaction && <React.Suspense fallback={null}><MerchantManagementTransactionDetailModal transaction={selectedTransaction} onClose={() => setSelectedTransaction(null)} /></React.Suspense>}
