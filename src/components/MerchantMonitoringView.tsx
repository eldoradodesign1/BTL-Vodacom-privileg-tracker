...
import { MerchantTransactionsDetailModal } from './Modals/MerchantTransactionsDetailModal';
...
export const MerchantMonitoringView: React.FC = () => {
  ...
  const [date, setDate] = useState(merchantTodayIso());
  ...
  const [visitedPosActivity, setVisitedPosActivity] = useState<MerchantTeamActivity | null>(null);
  const [transactionActivity, setTransactionActivity] = useState<MerchantTeamActivity | null>(null);
  ...
  const filteredTeam = useMemo(() => {
    ...
    return team.filter((item) => {
      ...
      const haystack = `${item.ba.name} ${item.ba.phone} ${item.mfsNames.join(' ')}`.toLowerCase();
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="glass-card space-y-3 p-4"><div className="flex items-center gap-3"><UsersRound className="text-cyan-200" size={20}/><div><h2 className="font-black">Équipe et performances</h2><p className="text-xs text-gray-400">{metrics.visited} POS visités · {metrics.transactions} transactions pour la date sélectionnée.</p></div></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(15rem,auto)_1fr]"><div className="flex items-center rounded-2xl border border-white/10 bg-white/[0.04] px-2 py-1"><DateIconPicker value={date} min={MERCHANT_CAMPAIGN_START} max={merchantTodayIso()} onChange={setDate} className="flex min-w-0 flex-1 items-center" buttonClassName="h-10 w-10 shrink-0 rounded-xl border border-cyan-300/20 bg-cyan-400/[0.08] text-cyan-100 transition hover:bg-cyan-400/15" labelClassName="truncate text-[10px] font-black uppercase text-gray-200" popoverAlign="left"/><button type="button" onClick={() => setDate(merchantTodayIso())} className={`ml-2 rounded-xl border px-2.5 py-2 text-[9px] font-black uppercase transition ${date === merchantTodayIso() ? 'border-cyan-300/50 bg-cyan-400/20 text-cyan-100' : 'border-white/10 bg-white/5 text-gray-400 hover:text-white'}`}>Aujourd’hui</button></div><div className="relative"><Search className="absolute left-3 top-3.5 text-gray-500" size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un BA ou un téléphone" className="app-input w-full rounded-2xl py-3 pl-10 pr-4 text-sm"/></div></div><div className="flex gap-2 overflow-x-auto pb-1">{([{ id: 'all', label: 'Tous' }, { id: 'present', label: 'En activité' }, { id: 'closed', label: 'Clôturés' }, { id: 'absent', label: 'Absents' }] as const).map((filter) => <button key={filter.id} onClick={() => setStatus(filter.id)} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-[10px] font-black uppercase ${status === filter.id ? 'border-cyan-300/50 bg-cyan-400/20 text-cyan-100' : 'border-white/10 bg-white/5 text-gray-400'}`}>{filter.label}</button>)}</div></section>
    <section className="space-y-3">{filteredTeam.map((item) => { const info = statusInfo(item); const checkin = item.attendance?.checkin_at ? new Date(item.attendance.checkin_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : '—'; const checkout = item.attendance?.checkout_at ? new Date(item.attendance.checkout_at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : '—'; const hasGps = typeof item.attendance?.checkin_latitude === 'number' || typeof item.attendance?.checkout_latitude === 'number'; return <article key={item.ba.id} className="glass-card p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><button type="button" onClick={() => setOperation({ mode: 'profile', activity: item })} className="block max-w-full truncate text-left font-black text-white transition hover:text-cyan-100">{item.ba.name}</button><p className="mt-0.5 text-xs text-gray-400">{item.ba.phone}</p>{item.mfsNames.length > 0 && <p className="mt-1 line-clamp-1 text-[10px] font-bold text-fuchsia-200">MFS · {item.mfsNames.join(' · ')}</p>}</div><div className="flex shrink-0 items-center gap-1.5">{actionButton(item, 'profile', 'Détail du BA', UserRound)}{actionButton(item, 'report', 'Aperçu du rapport', FileText, Boolean(item.attendance))}{actionButton(item, 'location', 'Localisation du pointage', MapPin, hasGps)}{actionButton(item, 'calendar', 'Registre de présence', CalendarDays)}<span className={`ml-1 inline-flex items-center gap-1 rounded-xl border px-2.5 py-1 text-[10px] font-black uppercase ${info.className}`}>{info.icon}{info.label}</span></div></div><div className="mt-4 grid grid-cols-3 gap-2 text-center"><button type="button" onClick={() => setVisitedPosActivity(item)} className="rounded-2xl border border-cyan-300/20 bg-cyan-400/[0.06] p-2 transition hover:border-cyan-300/45 hover:bg-cyan-400/[0.12]" title="Voir les POS visités"><Store className="mx-auto mb-0.5 text-cyan-100" size={13}/><b className="block text-sm text-cyan-100">{item.visitedPosCount}</b><span className="text-[8px] font-black uppercase text-gray-400">POS</span></button><button type="button" onClick={() => setTransactionActivity(item)} className="rounded-2xl border border-amber-300/20 bg-amber-500/[0.06] p-2 transition hover:border-amber-300/45 hover:bg-amber-500/[0.12]" title="Voir les transactions"><ReceiptText className="mx-auto mb-0.5 text-amber-100" size={13}/><b className="block text-sm text-amber-100">{item.transactionCount}</b><span className="text-[8px] font-black uppercase text-gray-400">Transactions</span></button><button type="button" onClick={() => setTransactionActivity(item)} className="rounded-2xl border border-emerald-300/20 bg-emerald-500/[0.06] p-2 transition hover:border-emerald-300/45 hover:bg-emerald-500/[0.12]" title="Voir les transactions et montants"><Banknote className="mx-auto mb-0.5 text-emerald-100" size={13}/><b className="block text-sm text-emerald-100">{item.totalAmount.toLocaleString('fr-FR')}</b><span className="text-[8px] font-black uppercase text-gray-400">Montant</span></button></div>{item.attendance && <div className="mt-3 flex items-center justify-between text-[10px] font-bold text-gray-400"><span>Arrivée {checkin}</span><span className="text-white/20">•</span><span>Clôture {checkout}</span><button onClick={() => setOperation({ mode: 'location', activity: item })} className="rounded-xl border border-white/10 bg-white/5 px-2 py-1 text-cyan-100 transition hover:bg-cyan-400/10">Voir la carte</button></div>}</article>; })}{filteredTeam.length === 0 && <div className="glass-card p-6 text-center text-sm text-gray-400">Aucun Brand Ambassador ne correspond aux filtres sélectionnés.</div>}</section>
    ...
    <MerchantVisitedPosModal isOpen={Boolean(visitedPosActivity)} activity={visitedPosActivity} run={run} onClose={() => setVisitedPosActivity(null)}/>
    <MerchantTransactionsDetailModal isOpen={Boolean(transactionActivity)} activity={transactionActivity} onClose={() => setTransactionActivity(null)}/>
  </div>;
};
