...
export const MpesaMikiliManagementView: React.FC<Props> = ({ currentUser, activeTab }) => {
  ...
  return (
    <div className="space-y-4 pb-6">
      ...
      {activeTab === 'tab3' && <>
        <section className="flex items-center justify-between gap-2 rounded-[1.5rem] border border-white/10 bg-white/[0.025] p-3"><div className="flex items-center gap-2"><Archive size={18} className="text-fuchsia-200"/><div><p className="text-[8px] font-black uppercase tracking-[0.18em] text-gray-500">Archives Mikili</p><h2 className="mt-0.5 text-sm font-black text-white">{dayLabel(date)} · {archiveClients.length} interactions</h2></div></div><DateIconPicker value={date} min={START_DATE} max={today} onChange={setDate} className="flex min-w-0 items-center" buttonClassName="h-9 w-9 shrink-0 rounded-xl border border-fuchsia-300/20 bg-fuchsia-500/10 text-fuchsia-100" labelClassName="hidden sm:block truncate text-[9px] font-black uppercase text-gray-300"/></section><section className="glass-card p-4"><div className="mt-0 grid grid-cols-2 gap-2"><div className="rounded-2xl bg-white/[0.04] p-3"><b className="block text-lg text-white">{archiveClients.filter((item) => item.activity_date === date).length}</b><span className="text-[8px] font-black uppercase text-gray-500">Clients du jour</span></div><div className="rounded-2xl bg-emerald-500/[0.05] p-3"><b className="block text-lg text-emerald-200">{archiveClients.filter((item) => item.activity_date === date && item.transaction_done).length}</b><span className="text-[8px] font-black uppercase text-gray-500">Transactions du jour</span></div></div></section>
