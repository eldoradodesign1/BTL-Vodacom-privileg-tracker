import{r as s,j as e,X as A,a7 as U,a8 as M,a9 as B,aa as G,ab as T,a3 as P,T as q,f as F}from"./vendor-react-DS-VKmM8.js";import{J as I,K as D,L as O,M as V,b as $,N as J,g as H,d as Q,j as X}from"./index-DZGnaEth.js";import"./vendor-supabase-CG6S1lgy.js";const W=`-- BTL Vodacom Privilege Tracker — schéma opérationnel
...
create table public.ba_transactions (
  ...
  comment text,
  status text
);`,Y=i=>`"${String(i??"").replace(/"/g,'""')}"`,ae=({isOpen:i,currentUser:c,onClose:N,onRefreshData:S})=>{const a=I(),[u,d]=s.useState((a==null?void 0:a.url)||""),[p,m]=s.useState((a==null?void 0:a.anonKey)||""),[h,f]=s.useState(""),[C,y]=s.useState(!!(a!=null&&a.geminiConfigured)),[z,_]=s.useState(!1),[v,j]=s.useState(!1),[x,l]=s.useState(null);if(s.useEffect(()=>{if(!i)return;let t=!0;return _(!0),D().then(r=>{t&&r&&(d(r.url),m(r.anonKey),y(r.geminiConfigured))}).catch(()=>{}).finally(()=>{t&&_(!1)}),()=>{t=!1}},[i]),!i)return null;const L=async()=>{if(c.role!=="super_admin"){l({type:"error",text:"Seul le super_admin peut modifier la configuration partagée."});return}if(!u.trim()||!p.trim()){l({type:"error",text:"Renseignez l’URL Supabase et la clé API publiable."});return}j(!0),l(null);try{new URL(u.trim());const t=await V({actor:{phone:c.phone,password:c.password},url:u,anonKey:p,geminiApiKey:h||void 0});d(t.url),m(t.anonKey),f(""),y(t.geminiConfigured),l({type:"success",text:"Configuration partagée mise à jour pour tous les appareils."}),S()}catch(t){l({type:"error",text:t instanceof Error?t.message:"La configuration partagée n’a pas pu être enregistrée."})}finally{j(!1)}},R=()=>{const r=[["users",$()],["shops",J()],["checkins",H()],["leads",Q()],["daily_reports",X()]].flatMap(([n,o])=>o.map(g=>({table:n,...g}))),k=Array.from(r.reduce((n,o)=>(Object.keys(o).forEach(g=>n.add(g)),n),new Set)),E=[k.join(","),...r.map(n=>k.map(o=>Y(n[o])).join(","))].join(`
