import{r as s,j as e,X as K,ag as U,ah as T,ai as B,aj as G,ak as M,ac as P,T as q,i as F}from"./vendor-react-DgaOdxTC.js";import{P as I,Q as D,R as O,S as V,b as $,T as Q,g as H,d as J,l as X}from"./index-DuKzFV76.js";import"./vendor-supabase-CG6S1lgy.js";const W=`-- BTL Vodacom Privilege Tracker — schéma opérationnel
...
create table public.ba_transactions (
  ...
  comment text,
  status text
);`,Y=i=>`"${String(i??"").replace(/"/g,'""')}"`,ae=({isOpen:i,currentUser:u,onClose:N,onRefreshData:S})=>{const a=I(),[c,d]=s.useState((a==null?void 0:a.url)||""),[p,m]=s.useState((a==null?void 0:a.anonKey)||""),[h,f]=s.useState(""),[C,y]=s.useState(!!(a!=null&&a.geminiConfigured)),[z,_]=s.useState(!1),[v,j]=s.useState(!1),[x,l]=s.useState(null);if(s.useEffect(()=>{if(!i)return;let t=!0;return _(!0),D().then(r=>{t&&r&&(d(r.url),m(r.anonKey),y(r.geminiConfigured))}).catch(()=>{}).finally(()=>{t&&_(!1)}),()=>{t=!1}},[i]),!i)return null;const R=async()=>{if(u.role!=="super_admin"){l({type:"error",text:"Seul le super_admin peut modifier la configuration partagée."});return}if(!c.trim()||!p.trim()){l({type:"error",text:"Renseignez l’URL Supabase et la clé API publiable."});return}j(!0),l(null);try{new URL(c.trim());const t=await V({actor:{phone:u.phone,password:u.password},url:c,anonKey:p,geminiApiKey:h||void 0});d(t.url),m(t.anonKey),f(""),y(t.geminiConfigured),l({type:"success",text:"Configuration partagée mise à jour pour tous les appareils."}),S()}catch(t){l({type:"error",text:t instanceof Error?t.message:"La configuration partagée n’a pas pu être enregistrée."})}finally{j(!1)}},L=()=>{const r=[["users",$()],["shops",Q()],["checkins",H()],["leads",J()],["daily_reports",X()]].flatMap(([n,o])=>o.map(g=>({table:n,...g}))),k=Array.from(r.reduce((n,o)=>(Object.keys(o).forEach(g=>n.add(g)),n),new Set)),E=[k.join(","),...r.map(n=>k.map(o=>Y(n[o])).join(","))].join(`
